...
class DataStudio {
    ...
    initializeGrid() {
        ...
        const columnDefs = window.columnDefsData.map((col, index) => {
            ...
            // Only the first column gets checkbox selection for row selection
            if (index === 0) {
                colDef.headerCheckboxSelection = false; // Disable built-in checkbox
                colDef.checkboxSelection = true;
                colDef.headerComponentParams = {
                    enableRowSelection: true, // Enable our custom checkbox
                    enableSorting: true
                };
            } else {
                colDef.headerComponentParams = {
    ...
    createCustomHeaderComponent() {
        ...
        CustomHeaderComponent.prototype.init = function(params) {
            ...
            // Enhanced horizontal flexbox layout with proper accessibility
            this.eGui.style.cssText = 'display: flex; align-items: center; justify-content: flex-start; height: 100%; padding: 6px 8px; gap: 8px; min-width: 0;';
            this.eGui.setAttribute('role', 'columnheader');
            this.eGui.setAttribute('aria-label', `Column ${params.displayName}`);
            
            // Add checkbox for column selection (always on the left when present)
            if (params.enableRowSelection) {
                ...
                // Enhanced checkbox interaction with proper event handling
                checkbox.addEventListener('change', (e) => {
                    e.stopPropagation();
                    // Prevent sorting when clicking checkbox
                    console.log('Column checkbox toggled:', params.displayName, e.target.checked);
                    // TODO: Integrate with improved dataStudio.selectColumn/deselectColumn
