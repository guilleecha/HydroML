...
class MLExperimentForm(forms.ModelForm):
    ...
    class Meta:
        model = MLExperiment
        # Añadimos el nuevo campo 'split_strategy' a la lista
        fields = [
        ...
        labels = {
            ...
            'test_split_size': 'Proporción de Test',
            'split_random_state': 'Semilla Aleatoria (Random State)',
        }
    ...
    def __init__(self, project, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Fieldset(
                ...
                'model_name',
                Div(
                    HTML('<p class="text-sm font-medium text-gray-600 mt-4">Hiperparámetros (Opcional)</p>'),
                    Row(Column('rf_n_estimators'), Column('rf_max_depth')),
                    id="rf-fields", css_class="hidden space-y-4 border-l-4 p-4"
                ),
                Div(
                    HTML('<p class="text-sm font-medium text-gray-600 mt-4">Hiperparámetros (Opcional)</p>'),
                    Row(Column('gb_n_estimators'), Column('gb_learning_rate')),
                    id="gb-fields", css_class="hidden space-y-4 border-l-4 p-4"
                ),
            ),
            Fieldset(
                'Selección de Datos y Variables',
                'input_datasource',
                # Añadimos el nuevo campo 'split_strategy' al layout
                Row(
                    Column('split_strategy', css_class='form-group col-md-4 mb-0'),
                    Column('test_split_size', css_class='form-group col-md-4 mb-0'),
                    Column('split_random_state', css_class='form-group col-md-4 mb-0'),
                ),
                HTML("""
                    <div class="mt-4">
                        <label for="id_target_column_select" class="block text-sm font-medium text-gray-700">Variable Objetivo</label>
                        <select id="id_target_column_select" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" disabled><option value="">Primero selecciona una Fuente de Datos</option></select>
                    </div>
                """),
                HTML("""
                    <div class="mt-6">
